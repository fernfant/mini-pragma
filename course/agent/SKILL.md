...
|---|---|---|---|
| 1 | **Interactivity** | 10 | Multiple meaningful click/drag/slide/run widgets; predict-before-reveal used at the key moment. |
| 2 | **Age-appropriate clarity** | 8 | Plain second-person voice; every term earned; short sentences; a 12–14-yo could follow unaided. |
...
| 5 | **Narrative continuity** | 4 | Re-uses the running example; connects back/forward; fits the spine story. |
| 6 | **Reinforcement** | 5 | Inline checks + recap + quiz; questions test understanding, not recall. |
...
| 14 | **Focus & cognitive load** | 7 | The page teaches **one** coherent idea with **one** running example and **one** consistent notation. 5 = a single example/symbol set carried throughout; concept count fits a single sitting. Penalize a second competing example, a symbol reused for two things (e.g. `X` = a single number in one widget and a 6-pixel vector in another), or more new load-bearing concepts than a 12–14-yo holds in one read. Emerges from *accumulated* additions, so re-score after every batch of edits, not just on first authoring. |
...
**Scoring #14 (Focus & cognitive load).** Cold-read the page and count three things: (1) distinct *running examples* — more than one competing for the spine is a red flag; (2) *notation collisions* — any symbol/letter that means two different things across the page (the L1.5 `X` = single number vs 6-pixel vector, which shipped at "96 Exemplary", is the canonical miss); (3) *new load-bearing concepts* — too many for one sitting. 5 = one example, one notation, a concept count a 12–14-yo holds in one read. 4 = slight scope creep (one extra concept, or a borderline second example). 3 = two genuinely competing examples, or a symbol reused with two meanings. ≤2 = the page is two lessons fused. The fix for a low #14 is usually a **split** (a new page), not edits — like #12 it's a roadmap signal, and the deduction lands on the overloaded page. **Why this criterion exists:** every other axis either rewards richness (Interactivity, Depth, Reinforcement — so adding a widget *raises* the score) or checks only *local* ordering (Flow). Nothing else penalises a page for doing too much, so overload accumulated undetected across sessions. Therefore **re-run #14 after any batch that adds a widget or section** — overload is a property of the *accumulated* page, not of any single edit.
...
>
> **Weights changed in the v14 revision** (to seat #14 Focus & cognitive load at 7): Interactivity 12→10, Clarity 9→8, Narrative 5→4, Reinforcement 6→5, Conceptual-depth 8→6. Pulled deliberately from the axes that *reward richness* (Interactivity, Depth) plus saturated ones — because rewarding richness with no opposing scope axis is exactly what let pages overload undetected (L1.5 sat at 96 while genuinely confusing a reader). New sum: Interactivity 10 · Clarity 8 · Concrete-first 9 · Accuracy 8 · Honesty 4 · Narrative 4 · Reinforcement 5 · Structure 5 · Technical 5 · Accessibility 8 · Flow 8 · Pacing 8 · Conceptual-depth 6 · Traceability 5 · Focus 7 = **100**.
